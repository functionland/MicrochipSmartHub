...
I2CSmartHubManager::I2CSmartHubManager(std::string &port_path, int i2c_address) : port_path_{port_path},
...
bool I2CSmartHubManager::RegisterRead(uint32_t address, uint16_t length, std::vector<uint8_t> &buffer)
{
    return false;
}
bool I2CSmartHubManager::RegisterWrite(uint32_t address, uint16_t length, const std::vector<uint8_t> &buffer)
{
    return false;
}
...
bool I2CSmartHubManager::WriteSmbus(uint8_t reg, std::vector<uint8_t> &buff)
{
    ...
    for (int i = 0; i < buff.size(); i++)
        wbuf[i] = buff[i];
    wbuf[buff.size()] = '\0';
    const auto res = i2c_smbus_write_block_data(file_handle_, reg, buff.size(), wbuf);
    if (res < 0)
    {
        LOG::Warn(TAG, "Can not write to reg:{} values:{1:#X}", reg, wbuf);
        return false;
    }
    return true;
}
